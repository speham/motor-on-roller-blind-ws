...
JsonVariant NidayandHelper::getconfig() { return this->_config; }
...
void NidayandHelper::mqtt_reconnect(PubSubClient &psclient,
                                    std::list<const char *> topics) {
  return mqtt_reconnect(psclient, String(NULL), String(NULL), topics);
}
void NidayandHelper::mqtt_reconnect(PubSubClient &psclient, String uid,
                                    String pwd) {
...
void NidayandHelper::mqtt_reconnect(PubSubClient &psclient, String uid,
  ...
  while (!psclient.connected()) {
    ...
    // Attempt to connect
    if ((mqttLogon ? psclient.connect(this->_mqttclientid.c_str(), uid.c_str(),
                                      pwd.c_str())
                   : psclient.connect(this->_mqttclientid.c_str()))) {
...
void NidayandHelper::mqtt_publish(PubSubClient &psclient, String topic,
                                  String payload) {
